...
struct Args {
    ...
    #[arg(long, default_value_t = 5)] jitter_samples: u32,
    #[arg(long, default_value_t = 0.18)] loom_strength: f64,
}
...
fn draw_label_box(img:&mut RgbaImage,x:i32,y:i32,scale_name:&str,ratio:&str,ss:u32){let title_scale=(7*ss.max(1))as i32;let ratio_scale=(4*ss.max(1))as i32;let text_w=((scale_name.len()as i32*6*title_scale).max(ratio.len()as i32*6*ratio_scale)).max(100*ss as i32);let w=text_w+40*ss as i32;let h=22*ss as i32+7*title_scale+16*ss as i32+7*ratio_scale+18*ss as i32;let margin=30*ss as i32;let x0=(x-w/2).clamp(margin,img.width()as i32-w-margin);let y0=(y-h/2).clamp(margin,img.height()as i32-h-margin);for yy in y0..y0+h{for xx in x0..x0+w{if xx>=0&&yy>=0&&xx<img.width()as i32&&yy<img.height()as i32{img.put_pixel(xx as u32,yy as u32,rgba((0,0,0),255));}}}let bw=(4*ss.max(1))as i32;for i in 0..bw{let c=rgba((58,58,58),255);draw_line_segment_mut(img,((x0+i)as f32,(y0+i)as f32),((x0+w-i)as f32,(y0+i)as f32),c);draw_line_segment_mut(img,((x0+i)as f32,(y0+h-i)as f32),((x0+w-i)as f32,(y0+h-i)as f32),c);draw_line_segment_mut(img,((x0+i)as f32,(y0+i)as f32),((x0+i)as f32,(y0+h-i)as f32),c);draw_line_segment_mut(img,((x0+w-i)as f32,(y0+i)as f32),((x0+w-i)as f32,(y0+h-i)as f32),c);}draw_text(img,scale_name,x0+20*ss as i32,y0+14*ss as i32,title_scale,rgba((238,238,238),255));draw_text(img,ratio,x0+20*ss as i32,y0+14*ss as i32+7*title_scale+14*ss as i32,ratio_scale,rgba((222,222,222),255));}
...
fn draw_micro_weave(img:&mut RgbaImage,w:u32,h:u32,name:&str){for y in(0..h).step_by(9){let c=if y%18==0{(24,20,18)}else{(14,18,18)};draw_line_segment_mut(img,(0.0,y as f32),(w as f32,y as f32),rgba(c,50));}for x in(0..w).step_by(13){let c=if x%26==0{(20,18,24)}else{(15,17,14)};draw_line_segment_mut(img,(x as f32,0.0),(x as f32,h as f32),rgba(c,30));}for i in 0..700{let x=(noise!(name,"speckx",i)*w as f64)as i32;let y=(noise!(name,"specky",i)*h as f64)as i32;let c=if noise!(name,"speckc",i)>0.5{(43,34,23)}else{(18,28,24)};draw_filled_circle_mut(img,(x,y),1,rgba(c,100));}}
fn draw_sector_tendrils(img:&mut RgbaImage,cx:f64,cy:f64,rr:f64,g:&Group,scale:&str,idx:usize){let col=dark(maqam_color(scale),0.25);let span=(g.end-g.start).max(0.01);for t in 0..7{let a=g.start+span*(t as f64+0.5)/7.0;let mut pts=Vec::new();for k in 0..10{let r=rr+62.0+k as f64*46.0;let aa=a+(noise!("tendril",idx,t,k)-0.5)*0.035;pts.push(Pt::new(cx+r*aa.cos(),cy+r*aa.sin()));}thread(img,&pts,col,3.0,120);}}

fn render(score:&Score,w:u32,h:u32,ss:u32)->RgbaImage{let mut img=RgbaImage::from_pixel(w,h,rgba((8,10,12),255));for y in(0..h).step_by(5){for x in(0..w).step_by(5){let v=noise!(&score.name,"field",x/5,y/5);let c=if v>0.70{(25,22,20)}else if v>0.45{(18,23,22)}else{(16,15,14)};img.put_pixel(x,y,rgba(c,255));}}draw_micro_weave(&mut img,w,h,&score.name);let rect=(w as f64*0.18,h as f64*0.20,w as f64*0.82,h as f64*0.80);let cx=(rect.0+rect.2)/2.0;let cy=(rect.1+rect.3)/2.0;let rr=((rect.2-rect.0).min(rect.3-rect.1))*0.60;let gos=fit(&gosper(4),rect,20.0*ss as f64);thread(&mut img,&gos,(5,5,5),7.0*ss as f64,255);thread(&mut img,&gos,(18,14,10),3.0*ss as f64,230);let gs=groups(score);let map:HashMap<i32,&Phrase>=score.phrases.iter().map(|p|(p.id,p)).collect();for(gi,g)in gs.iter().enumerate(){let Some(p)=map.get(&g.phrase_id)else{continue;};let scale=p.voices.first().map(|v|v.scale.as_str()).unwrap_or("rast");let col=maqam_color(scale);draw_sector_tendrils(&mut img,cx,cy,rr,g,scale,gi);draw_arc(&mut img,cx,cy,rr,g.start,g.end,(96,76,44),18.0*ss as f64);draw_arc(&mut img,cx,cy,rr-18.0*ss as f64,g.start,g.end,(26,21,16),4.0*ss as f64);for ring in 0..7{let r2=rr+58.0*ss as f64+ring as f64*50.0*ss as f64;let count=5+ring*2;for k in 0..count{let a=g.start+(g.end-g.start)*(k as f64+0.5)/count as f64;let jitter=(noise!("motif",&score.name,gi,ring,k)-0.5)*18.0*ss as f64;let x=cx+(r2+jitter)*a.cos();let y=cy+(r2+jitter)*a.sin();let s=if ring%2==0{7}else{5};draw_filled_circle_mut(&mut img,(x as i32,y as i32),(s+5)*ss as i32,rgba(dark(col,0.55),180));draw_filled_circle_mut(&mut img,(x as i32,y as i32),s*ss as i32,rgba(light(col,0.10),215));}}for(a,is_kick)in g.angles.iter().zip(g.kinds.iter()){let rdot=if*is_kick{20}else{10};let x=cx+(rr+35.0*ss as f64)*a.cos();let y=cy+(rr+35.0*ss as f64)*a.sin();draw_filled_circle_mut(&mut img,(x as i32,y as i32),(rdot+5)*ss as i32,rgba((18,15,12),255));draw_filled_circle_mut(&mut img,(x as i32,y as i32),rdot*ss as i32,rgba(col,255));}let mid=(g.start+g.end)/2.0;draw_label_box(&mut img,(cx+(rr+176.0*ss as f64)*mid.cos())as i32,(cy+(rr+176.0*ss as f64)*mid.sin())as i32,&scale.to_uppercase(),&ratio_text(score,scale),ss);}draw_jumps(&mut img,score,cx,cy,rr,&gs);img}
...
fn jittered_sample(img:&RgbaImage,fx:f64,fy:f64,samples:u32,name:&str,x:u32,y:u32)->Rgba<u8>{let n=samples.max(1);let mut acc=[0.0;4];for i in 0..n{let jx=(noise!(name,"jx",x,y,i)-0.5)*1.1;let jy=(noise!(name,"jy",x,y,i)-0.5)*1.1;let p=sample_bilinear(img,fx+jx,fy+jy).0;for k in 0..4{acc[k]+=p[k]as f64;}}Rgba([(acc[0]/n as f64).round().clamp(0.0,255.0)as u8,(acc[1]/n as f64).round().clamp(0.0,255.0)as u8,(acc[2]/n as f64).round().clamp(0.0,255.0)as u8,(acc[3]/n as f64).round().clamp(0.0,255.0)as u8])}
...
fn loom_overlay(img:&mut RgbaImage,strength:f64){if strength<=0.0{return;}let w=img.width();let h=img.height();let a1=(8.0*strength).round().clamp(0.0,16.0)as u8;let a2=(6.0*strength).round().clamp(0.0,14.0)as u8;for y in(0..h).step_by(5){draw_line_segment_mut(img,(0.0,y as f32),(w as f32,y as f32),rgba((26,22,18),a1));}for x in(0..w).step_by(7){draw_line_segment_mut(img,(x as f32,0.0),(x as f32,h as f32),rgba((12,12,12),a2));}}
fn carpet(design:&RgbaImage,kw:u32,kh:u32,kp:u32,samples:u32,name:&str,loom_strength:f64)->RgbaImage{let mut out=RgbaImage::from_pixel(kw*kp,kh*kp,rgba((12,11,10),255));for y in 0..kh{for x in 0..kw{let fx=(x as f64+0.5)*design.width()as f64/kw as f64-0.5;let fy=(y as f64+0.5)*design.height()as f64/kh as f64-0.5;let mut p=jittered_sample(design,fx,fy,samples,name,x,y);let v=(noise!("knot",x,y)-0.5)*8.0;if lum(p)>18.0&&lum(p)<155.0{for c in 0..3{p[c]=(p[c]as f64+v).round().clamp(0.0,255.0)as u8;}}p=loom_tint(p,x,y,loom_strength);let cx=(x*kp+kp/2)as i32;let cy=(y*kp+kp/2)as i32;let px=kp as i32;let poly=[Point::new(cx,cy-px/2),Point::new(cx+px/2,cy),Point::new(cx,cy+px/2),Point::new(cx-px/2,cy)];draw_polygon_mut(&mut out,&poly,p);}}loom_overlay(&mut out,loom_strength);out}
