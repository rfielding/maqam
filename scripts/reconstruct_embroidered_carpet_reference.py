#!/usr/bin/env python3
"""
reconstruct_embroidered_carpet_reference.py

Recovered procedural reference for the embroidered maqam carpet idea.
This is a source-only renderer: no input image is loaded. It generates pixels
from a .mq session file and is intended as a porting reference for the Rust
`source_background.rs` renderer.

Usage:
  python3 scripts/reconstruct_embroidered_carpet_reference.py magiccarpet.mq out/carpet.png
  python3 scripts/reconstruct_embroidered_carpet_reference.py --demo out/carpet.png

Dependencies:
  pip install pillow numpy

Notes:
  This file is reconstructed from File Library snippets of the old Python
  `reconstruct_embroidered_carpet_no_flowers.py`. It is meant to preserve the
  visual algorithm: dark textile base, organic phrase territories, beaded seams,
  rhythm bands, ratio constellations, angular cartouches, and all-over stitches.
"""
...
def parse_session(text: str) -> Session:
    ...
    for idx, raw in enumerate(text.splitlines()):
        line = raw.strip()
        if not line or line.startswith("MAQAM_SESSION"):
            continue
        toks = line.split()
        if not toks:
            continue
...
def render(session: Session, out: Path, seed=1701):
    ...
    phrases = session.phrases or [Phrase(0, "g hijaz 4444", "g hijaz\n4444", ["hijaz"], "4444")]
    n = max(1, len(phrases))
    if n == 3:
    ...
    for i, ph in enumerate(phrases):
        ...
        groups = [int(c) for c in ph.rhythm if c.isdigit()] or [4, 4]
    ...
    for j in session.jumps:
        if not blobs:
            continue
        src_i = max(0, min(len(blobs)-1, sum(1 for ph in phrases if ph.idx < j.idx)-1))
        dst_i = next((k for k, ph in enumerate(phrases) if ph.idx == j.target), j.target % len(blobs))
        sx, sy = blobs[src_i][3]
        dx, dy = blobs[dst_i][3]
        mx, my = (sx+dx)/2, (sy+dy)/2
        for k in range(max(1, min(12, j.count * 2))):
            a = math.tau * k / max(1, min(12, j.count * 2))
            d.ellipse((mx+math.cos(a)*18-3, my+math.sin(a)*18-3, mx+math.cos(a)*18+3, my+math.sin(a)*18+3), fill=ACCENT["orange"]+(170,))
...
def main():
    ...
    if args.demo:
        ...
        if not args.input:
            raise SystemExit("usage: reconstruct_embroidered_carpet_reference.py <session.mq> <out.png> or --demo <out.png>")
