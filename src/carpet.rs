...
fn draw_ring_score(buf:&mut[u8],w:usize,score:&WeaveScore){ dot(buf,w,CX.round()as i32,CY.round()as i32,INNER_R.round()as i32+14,[0,0,0],0.78); dot(buf,w,CX.round()as i32,CY.round()as i32,INNER_R.round()as i32-4,[6,5,4],0.64); let layout=score_border_layout(score); for phrase in &score.phrases{ let ticks:Vec<_>=layout.iter().filter(|t|t.phrase_id==phrase.phrase_id).collect(); if let(Some(first),Some(last))=(ticks.first(),ticks.last()){let color=phrase_color(phrase.phrase_id); draw_arc(buf,w,INNER_R,first.start_t,last.end_t,dark(color,0.34),0.84,13); draw_arc(buf,w,INNER_R-16.0,first.start_t,last.end_t,[22,17,12],0.68,4);} } for tick in layout{ let c=if tick.is_kick{[194,142,80]}else{light(dark(phrase_color(tick.phrase_id),0.20),0.08)}; dot(buf,w,tick.x.round()as i32,tick.y.round()as i32,if tick.is_kick{12}else{7},[6,4,3],0.96); dot(buf,w,tick.x.round()as i32,tick.y.round()as i32,if tick.is_kick{8}else{5},c,0.90); } }
fn draw_jump_arrows(buf:&mut[u8],w:usize,phrases:&[Phrase],score:&WeaveScore){ for(order,route)in jump_routes(phrases,score).into_iter().enumerate(){ let r=(INNER_R-22.0-order as f32*18.0).max(58.0); let mut target_t=route.target_t; while target_t>=route.source_t{target_t-=1.0;} let color=if order%2==0{[170,126,72]}else{[150,104,60]}; draw_arc(buf,w,r,route.source_t,target_t,[5,4,3],0.90,8); draw_arc(buf,w,r,route.source_t,target_t,color,0.78,4); let tip=point_on_circle(BEAT_R,route.target_t); dot(buf,w,tip.x.round()as i32,tip.y.round()as i32,5,color,0.82); } }
