...
fn dot(buf: &mut [u8], w: usize, cx: i32, cy: i32, r: i32, rgb: [u8; 3], a: f32) {
    ...
    for y in cy-r..=cy+r { for x in cx-r..=cx+r {
        let dx = (x - cx) as f32; let dy = (y - cy) as f32; let d2 = dx*dx + dy*dy;
        if d2 <= rr { let falloff = 0.35 + 0.65 * (1.0 - (d2 / rr).sqrt()); blend_px(buf, w, x, y, rgb, a * falloff); }
    }}
}
...
fn phrase_color(phrase_id: usize) -> [u8; 3] {
    const COLORS: [[u8;3]; 8] = [[88,44,122],[32,110,78],[30,104,132],[148,74,46],[132,105,42],[78,92,126],[118,70,112],[108,78,48]];
...
fn hilbert_d2xy(order: u32, mut d: i32) -> Pt {
    let n = 1_i32 << order; let mut x = 0_i32; let mut y = 0_i32; let mut s = 1_i32;
    while s < n { let rx = 1 & (d / 2); let ry = 1 & (d ^ rx); hilbert_rot(s, &mut x, &mut y, rx, ry); x += s * rx; y += s * ry; d /= 4; s *= 2; }
...
fn fit_points(pts: &[Pt], x0: f32, y0: f32, x1: f32, y1: f32, margin: f32) -> Vec<Pt> {
    let minx=pts.iter().map(|p|p.x).fold(f32::INFINITY,f32::min); let maxx=pts.iter().map(|p|p.x).fold(f32::NEG_INFINITY,f32::max);
    let miny=pts.iter().map(|p|p.y).fold(f32::INFINITY,f32::min); let maxy=pts.iter().map(|p|p.y).fold(f32::NEG_INFINITY,f32::max);
    let sx=(x1-x0-2.0*margin)/(maxx-minx).max(0.001); let sy=(y1-y0-2.0*margin)/(maxy-miny).max(0.001); let sc=sx.min(sy);
    let ox=x0+(x1-x0-(maxx-minx)*sc)/2.0-minx*sc; let oy=y0+(y1-y0-(maxy-miny)*sc)/2.0-miny*sc;
...
pub fn score_border_layout(score: &WeaveScore) -> Vec<BorderTickLayout> {
    ...
    for (phrase_index, phrase) in score.phrases.iter().enumerate() {
        ...
        for score_tick in 0..phrase.tick_count {
            ...
            layout.push(BorderTickLayout { phrase_id: phrase.phrase_id, score_tick, x: p.x, y: p.y, is_kick: tick.tick_in_group == 0, start_t, end_t });
        }
    }
    layout
}
...
fn jump_routes(phrases: &[Phrase], score: &WeaveScore) -> Vec<JumpRoute> {
    ...
    for phrase in &score.phrases {
        ...
        if let (Some(first), Some(last)) = (phrase_ticks.first(), phrase_ticks.last()) { bounds.insert(phrase.phrase_id, (first.start_t, last.end_t)); }
    }
    let musical_ids: Vec<usize> = phrases.iter().filter(|phrase| phrase.jump.is_none() && phrase.control.is_none()).map(|phrase| phrase.id).collect();
    ...
    let mut previous_phrase_id = None; let mut routes = Vec::new();
    for phrase in phrases {
        ...
        } else if phrase.control.is_none() { previous_phrase_id = Some(phrase.id); }
    }
    routes
}
...
pub fn jump_link_cells(phrases: &[Phrase]) -> Vec<JumpLinkCell> {
    ...
    for (order, route) in routes.into_iter().enumerate() {
        ...
        let mut target_t = route.target_t; while target_t >= route.source_t { target_t -= 1.0; }
...
pub fn jump_counter_layout(phrases: &[Phrase]) -> Vec<JumpCounterLayout> {
    let score = WeaveScore::from_phrases(phrases);
    jump_routes(phrases, &score).into_iter().enumerate().map(|(order, route)| {
        let r = (INNER_R - 48.0 - order as f32 * 20.0).max(46.0);
        let mut target_t = route.target_t;
        while target_t >= route.source_t { target_t -= 1.0; }
        let mid_t = norm_t(route.source_t + (target_t - route.source_t) * 0.5);
        let p = point_on_circle(r, mid_t);
        JumpCounterLayout { jump_id: route.jump_id, x: p.x, y: p.y }
    }).collect()
}

fn draw_arc(buf: &mut [u8], w: usize, r: f32, t0: f32, t1: f32, rgb: [u8;3], a: f32, thick: i32) {
    let mut end = t1; if end < t0 { end += 1.0; }
    let steps = ((end - t0).abs() * 180.0).ceil().max(8.0) as usize;
    let mut prev = point_on_circle(r, t0);
    for i in 1..=steps { let t=t0+(end-t0)*i as f32/steps as f32; let p=point_on_circle(r,norm_t(t)); thick_line(buf,w,prev.x.round() as i32,prev.y.round() as i32,p.x.round() as i32,p.y.round() as i32,rgb,a,thick); prev=p; }
}
fn draw_base_weave(buf: &mut [u8], w: usize) {
    for y in 0..CARPET_H { for x in 0..w {
        let n=hash01((x as u32).wrapping_mul(73856093) ^ (y as u32).wrapping_mul(19349663));
        let warp=if x%4<2{1.08}else{0.92}; let weft=if y%4<2{1.04}else{0.96}; let base=[10.0,9.0,8.0];
        put_px(buf,w,x,y,[(base[0]*warp*weft+n*5.0) as u8,(base[1]*warp*weft+n*4.0) as u8,(base[2]*warp*weft+n*3.0) as u8]);
    }}
}
fn draw_outer_hilbert(buf: &mut [u8], w: usize, score: &WeaveScore) {
    // 2x virtual canvas: the curve is physically larger and clips across the full carpet.
    let pts = fit_points(&hilbert_points(6), -(w as f32)*0.5, -(CARPET_H as f32)*0.5, (w as f32)*1.5, (CARPET_H as f32)*1.5, 0.0);
    let clear_r2 = (BEAT_R + 34.0) * (BEAT_R + 34.0);
    for seg in pts.windows(2) {
        let a=seg[0]; let b=seg[1]; let m=a.add(b).mul(0.5); let dx=m.x-CX; let dy=m.y-CY;
        if dx*dx + dy*dy <= clear_r2 { continue; }
        let t=angle_t(m.x,m.y); let phrase_idx=((t*score.phrases.len().max(1) as f32).floor() as usize).min(score.phrases.len().saturating_sub(1));
        let phrase_id=score.phrases.get(phrase_idx).map(|p|p.phrase_id).unwrap_or(0); let c=dark(phrase_color(phrase_id),0.35);
        thick_line(buf,w,a.x.round() as i32,a.y.round() as i32,b.x.round() as i32,b.y.round() as i32,dark(c,0.65),0.34,5);
        thick_line(buf,w,a.x.round() as i32,a.y.round() as i32,b.x.round() as i32,b.y.round() as i32,c,0.62,3);
    }
}
fn draw_ring_score(buf: &mut [u8], w: usize, score: &WeaveScore) {
    ...
    let layout = score_border_layout(score);
    for phrase in &score.phrases {
        let ticks: Vec<_> = layout.iter().filter(|t| t.phrase_id == phrase.phrase_id).collect();
        if let (Some(first), Some(last)) = (ticks.first(), ticks.last()) { let color=phrase_color(phrase.phrase_id); draw_arc(buf,w,INNER_R,first.start_t,last.end_t,dark(color,0.18),0.92,13); draw_arc(buf,w,INNER_R-16.0,first.start_t,last.end_t,[28,22,16],0.75,4); }
    }
    for tick in layout { let c=if tick.is_kick{[214,166,94]}else{light(phrase_color(tick.phrase_id),0.18)}; dot(buf,w,tick.x.round() as i32,tick.y.round() as i32,if tick.is_kick{12}else{7},[8,6,4],0.95); dot(buf,w,tick.x.round() as i32,tick.y.round() as i32,if tick.is_kick{8}else{5},c,0.96); }
}
fn draw_jump_arrows(buf: &mut [u8], w: usize, phrases: &[Phrase], score: &WeaveScore) {
    for (order, route) in jump_routes(phrases, score).into_iter().enumerate() {
        let r=(INNER_R-22.0-order as f32*18.0).max(58.0); let mut target_t=route.target_t; while target_t>=route.source_t{target_t-=1.0;}
        let color=if order%2==0{[218,174,108]}else{[190,142,82]}; draw_arc(buf,w,r,route.source_t,target_t,[7,5,3],0.90,8); draw_arc(buf,w,r,route.source_t,target_t,color,0.90,4); let tip=point_on_circle(BEAT_R,route.target_t); dot(buf,w,tip.x.round() as i32,tip.y.round() as i32,5,color,0.95);
    }
}
fn draw_frame(buf: &mut [u8], w: usize) {
    for x in (24..w-24).step_by(8) { let len=15+(hash01(x as u32)*18.0) as i32; line(buf,w,x as i32,28,x as i32+(hash01(x as u32+7)*12.0-6.0) as i32,28-len,[118,100,76],0.72); line(buf,w,x as i32,692,x as i32+(hash01(x as u32+13)*12.0-6.0) as i32,692+len,[118,100,76],0.72); }
    for y in (36..CARPET_H-36).step_by(8) { let len=12+(hash01(y as u32+99)*16.0) as i32; line(buf,w,38,y as i32,38-len,y as i32+(hash01(y as u32+3)*12.0-6.0) as i32,[118,100,76],0.62); line(buf,w,w as i32-38,y as i32,w as i32-38+len,y as i32+(hash01(y as u32+5)*12.0-6.0) as i32,[118,100,76],0.62); }
    thick_line(buf,w,52,40,w as i32-52,40,[72,48,30],0.70,8); thick_line(buf,w,52,680,w as i32-52,680,[72,48,30],0.70,8); thick_line(buf,w,52,40,52,680,[72,48,30],0.70,8); thick_line(buf,w,w as i32-52,40,w as i32-52,680,[72,48,30],0.70,8);
}
...
mod tests {
    ...
    fn score_cycles_match_border_tick_example() {
        ...
        let total: usize = canonical_score_groups(&[4, 4, 4, 4]).into_iter().chain(canonical_score_groups(&[3, 3, 2])).map(usize::from).sum();
        assert_eq!(total, 24);
    }
    #[test]
    fn circular_layout_has_points() {
        ...
        for tick in layout { assert!((tick.x - CX).abs() <= BEAT_R + 2.0); assert!((tick.y - CY).abs() <= BEAT_R + 2.0); }
    }
}
