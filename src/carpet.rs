...
//
// Keep the carpet/video visual code out of record.rs.  The invariant is that
// playback state should guide the eye through a dark woven manuscript; it
// should not add GUI boxes, cursors, piano rolls, or waveform geometry.
...
fn cents_from_root(hz: f64, root_hz: f64) -> f64 {
    if hz <= 0.0 || root_hz <= 0.0 { return 0.0; }
...
fn fill_background(buf: &mut [u8], w: usize) {
    ...
    for x in (0..w).step_by(18) {
        let c = if (x / 18) % 2 == 0 { [26, 20, 42] } else { [16, 39, 34] };
        fill_rect(buf, w, x, 12, 10, 18, c);
        fill_rect(buf, w, x, CARPET_H - 32, 10, 18, c);
    }
    for x in (0..w).step_by(43) {
        thick_line(buf, w, x as i32, 88, x as i32 + 22, 116, [76, 52, 92], 0.28, 2);
        thick_line(buf, w, x as i32, 604, x as i32 + 22, 632, [55, 84, 80], 0.24, 2);
    }
}
...
fn draw_region_cell(buf: &mut [u8], w: usize, x: i32, y: i32, idx: usize, active: bool) {
    ...
    thick_line(buf, w, x - 26, y, x - 11, y - 18, edge, if active { 0.55 } else { 0.26 }, if active { 3 } else { 1 });
    thick_line(buf, w, x - 11, y - 18, x + 18, y - 16, edge, if active { 0.55 } else { 0.26 }, if active { 3 } else { 1 });
    thick_line(buf, w, x + 18, y - 16, x + 27, y + 3, edge, if active { 0.55 } else { 0.26 }, if active { 3 } else { 1 });
    thick_line(buf, w, x + 27, y + 3, x + 8, y + 20, edge, if active { 0.55 } else { 0.26 }, if active { 3 } else { 1 });
    thick_line(buf, w, x + 8, y + 20, x - 22, y + 15, edge, if active { 0.55 } else { 0.26 }, if active { 3 } else { 1 });
    thick_line(buf, w, x - 22, y + 15, x - 26, y, edge, if active { 0.55 } else { 0.26 }, if active { 3 } else { 1 });
}

fn draw_stitches(buf: &mut [u8], w: usize, x: i32, y: i32, seed: u32, root_hz: f64, hz: f64, active: bool) {
    let cents = cents_from_root(hz, root_hz) as f32;
    let base_ang = cents / 1200.0 * std::f32::consts::TAU;
    let colors = [[220, 176, 88], [90, 184, 162], [150, 96, 196], [190, 82, 73]];
    for i in 0..16u32 {
        ...
        dot(buf, w, px, py, if active { 3 } else { 2 }, c, if active { 0.66 } else { 0.28 });
    }
}

pub fn write_carpet_background(path: impl AsRef<std::path::Path>, entries: &[CarpetEntry], phrases: &[Phrase]) -> anyhow::Result<CarpetRenderInfo> {
    let mut tick_count = 0usize;
    for e in entries {
        tick_count += phrases[e.phrase_idx].bar.events.len().max(1);
    }
    let step_x = 30usize;
    let w = (tick_count * step_x + 760).max(1600);
    ...
    for e in entries {
        ...
        for ev in &bar.events {
            let hz = event_hz(*ev);
            let row = bar.frequencies.iter().enumerate().min_by(|(_, a), (_, b)| {
            ...
            for r in 0..rows {
                let top = 150i32;
                let bottom = 545i32;
                let y = top + ((bottom - top) as f32 * ((r + 1) as f32 / (rows + 1) as f32)).round() as i32;
                let wave = ((tick as f32 * 0.41) + r as f32 * 0.73).sin();
                let active = r == row;
                draw_region_cell(&mut buf, w, x, y + (wave * 13.0) as i32, e.phrase_idx + r, active);
                if active {
                    draw_stitches(&mut buf, w, x, y, tick as u32, bar.root_hz, hz, true);
                }
            }
            if tick > 0 {
                thick_line(&mut buf, w, x - step_x as i32, 350, x, 350 + ((tick as f32 * 0.37).sin() * 44.0) as i32, [96, 76, 58], 0.18, 2);
            }
            x += step_x as i32;
            tick += 1;
        }
    ...
    Ok(CarpetRenderInfo { path: path.to_string_lossy().replace('\\', "/"), width: w, content_end_x: x.max(1180) as usize })
}
