...
fn draw_region_cell(buf: &mut [u8], w: usize, x: i32, y: i32, idx: usize, active: bool) {
    let palette = [[62, 30, 82], [22, 70, 54], [18, 66, 86], [92, 48, 30], [72, 60, 24]];
    let edge = [158, 126, 74];
    let fill = palette[idx % palette.len()];
    let a = if active { 0.58 } else { 0.30 };
    for yy in -18i32..=18i32 {
        let span = 26 - (yy.abs() / 2);
        for xx in -span..=span { blend_px(buf, w, x + xx, y + yy, fill, a * 0.75); }
    }
...
pub fn write_carpet_background(path: impl AsRef<std::path::Path>, entries: &[CarpetEntry], phrases: &[Phrase]) -> anyhow::Result<CarpetRenderInfo> {
    ...
    for e in entries { tick_count += phrases[e.phrase_idx].bar.events.len().max(1); }
    ...
    for e in entries {
        ...
        for ev in &bar.events {
            ...
            for r in 0..rows {
                ...
                if active { draw_stitches(&mut buf, w, x, y, tick as u32, bar.root_hz, hz); }
            }
            if tick > 0 {
                thick_line(&mut buf, w, x - CARPET_STEP_X as i32, 350, x, 350 + ((tick as f32 * 0.37).sin() * 44.0) as i32, [96, 76, 58], 0.18, 2);
            }
    ...
    f.write_all(&buf)?;
    f.flush()?;
    Ok(CarpetRenderInfo { path: path.to_string_lossy().replace('\\', "/"), width: w, content_end_x: x.max(CARPET_START_X as i32) as usize })
}
