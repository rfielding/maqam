...
fn line(buf: &mut [u8], w: usize, mut x0: i32, mut y0: i32, x1: i32, y1: i32, rgb: [u8; 3], a: f32) { let dx = (x1 - x0).abs(); let sx = if x0 < x1 { 1 } else { -1 }; let dy = -(y1 - y0).abs(); let sy = if y0 < y1 { 1 } else { -1 }; let mut err = dx + dy; loop { blend_px(buf, w, x0, y0, rgb, a); if x0 == x1 && y0 == y1 { break; } let e2 = err * 2; if e2 >= dy { err += dy; x0 += sx; } if e2 <= dx { err += dx; y0 += sy; } } }
...
fn dot(buf: &mut [u8], w: usize, cx: i32, cy: i32, r: i32, rgb: [u8; 3], a: f32) { let r = r.max(1); let rr = (r * r) as f32; for y in cy-r..=cy+r { for x in cx-r..=cx+r { let dx = (x - cx) as f32; let dy = (y - cy) as f32; let d2 = dx*dx + dy*dy; if d2 <= rr { let falloff = 0.35 + 0.65 * (1.0 - (d2 / rr).sqrt()); blend_px(buf, w, x, y, rgb, a * falloff); } } } }
...
fn draw_label_box(buf: &mut [u8], w: usize, x: i32, y: i32, title: &str, ratios: &str) { let ts = 2; let rs = 1; let tw = text_width(title, ts).max(text_width(ratios, rs)).max(48); let bw = tw + 16; let bh = 8 + 7*ts + 5 + 7*rs + 8; let mut x0 = x - bw/2; let mut y0 = y - bh/2; x0 = x0.clamp(74, w as i32 - bw - 74); y0 = y0.clamp(58, CARPET_H as i32 - bh - 58); for yy in y0..y0+bh { for xx in x0..x0+bw { blend_px(buf,w,xx,yy,[0,0,0],0.96); } } thick_line(buf,w,x0,y0,x0+bw,y0,[48,36,18],0.82,2); thick_line(buf,w,x0,y0+bh,x0+bw,y0+bh,[48,36,18],0.82,2); thick_line(buf,w,x0,y0,x0,y0+bh,[48,36,18],0.82,2); thick_line(buf,w,x0+bw,y0,x0+bw,y0+bh,[48,36,18],0.82,2); draw_text(buf,w,title,x0+8,y0+7,ts,[150,112,54],0.78); draw_text(buf,w,ratios,x0+8,y0+7+7*ts+5,rs,[118,88,42],0.72); }
...
fn gosper_expand(symbols: &str) -> String { let mut out = String::with_capacity(symbols.len() * 8); for ch in symbols.chars() { match ch { 'A' => out.push_str("A-B--B+A++AA+B-"), 'B' => out.push_str("+A-BB--B-A++A+B"), '+' | '-' => out.push(ch), _ => {} } } out }
fn gosper_points(levels: usize, step: f32) -> Vec<Pt> { let mut prog = "A".to_string(); for _ in 0..levels { prog = gosper_expand(&prog); } let mut pts = Vec::new(); let mut p = Pt::new(0.0, 0.0); let mut ang = 0.0f32; let turn = std::f32::consts::PI / 3.0; pts.push(p); for ch in prog.chars() { match ch { 'A' | 'B' => { p = Pt::new(p.x + step * ang.cos(), p.y + step * ang.sin()); pts.push(p); }, '+' => ang += turn, '-' => ang -= turn, _ => {} } } pts }
fn fit_points_to_circle(pts: &[Pt], cx: f32, cy: f32, r: f32, margin: f32) -> Vec<Pt> { let minx=pts.iter().map(|p|p.x).fold(f32::INFINITY,f32::min); let maxx=pts.iter().map(|p|p.x).fold(f32::NEG_INFINITY,f32::max); let miny=pts.iter().map(|p|p.y).fold(f32::INFINITY,f32::min); let maxy=pts.iter().map(|p|p.y).fold(f32::NEG_INFINITY,f32::max); let scale=((2.0*(r-margin))/(maxx-minx).max(maxy-miny).max(0.001)).max(0.001); let ox=cx-((minx+maxx)*0.5)*scale; let oy=cy-((miny+maxy)*0.5)*scale; pts.iter().map(|p|Pt::new(ox+p.x*scale,oy+p.y*scale)).collect() }
fn draw_center_gosper(buf: &mut [u8], w: usize, score: &WeaveScore) { let thread = score.phrases.first().map(|p| dark(phrase_color(p.phrase_id),0.55)).unwrap_or([74,54,24]); let pts = fit_points_to_circle(&gosper_points(4, 1.0), CX, CY, INNER_R - 30.0, 10.0); let clip = INNER_R - 20.0; let clip2 = clip * clip; for seg in pts.windows(2) { let a=seg[0]; let b=seg[1]; let mx=(a.x+b.x)*0.5; let my=(a.y+b.y)*0.5; let dx=mx-CX; let dy=my-CY; if dx*dx+dy*dy > clip2 { continue; } thick_line(buf,w,a.x.round()as i32,a.y.round()as i32,b.x.round()as i32,b.y.round()as i32,dark(thread,0.68),0.48,4); thick_line(buf,w,a.x.round()as i32,a.y.round()as i32,b.x.round()as i32,b.y.round()as i32,thread,0.64,2); } }

fn draw_chevron(buf:&mut[u8],w:usize,r:f32,t:f32,rgb:[u8;3],a:f32,scale:f32,ccw:bool){ let ang=(t-0.25)*TAU; let p=point_on_circle(r,norm_t(t)); let (dx,dy)=if ccw{(ang.sin(),-ang.cos())}else{(-ang.sin(),ang.cos())}; let nx=-dy; let ny=dx; let ax=p.x+dx*scale; let ay=p.y+dy*scale; let bx=p.x-dx*scale*0.8; let by=p.y-dy*scale*0.8; let l=Pt::new(bx+nx*scale*0.65,by+ny*scale*0.65); let rr=Pt::new(bx-nx*scale*0.65,by-ny*scale*0.65); thick_line(buf,w,l.x.round()as i32,l.y.round()as i32,ax.round()as i32,ay.round()as i32,rgb,a,2); thick_line(buf,w,rr.x.round()as i32,rr.y.round()as i32,ax.round()as i32,ay.round()as i32,rgb,a,2); }
fn draw_chevrons(buf:&mut[u8],w:usize,r:f32,t0:f32,t1:f32,rgb:[u8;3],a:f32,scale:f32,ccw:bool){ if ccw{ let mut end=t1; while end>=t0{end-=1.0;} let span=(t0-end).abs(); let steps=(span*70.0).ceil().max(3.0)as usize; for i in 1..steps{let t=t0-span*i as f32/steps as f32; draw_chevron(buf,w,r,t,rgb,a,scale,true);} } else { let mut end=t1; if end<t0{end+=1.0;} let span=(end-t0).abs(); let steps=(span*70.0).ceil().max(3.0)as usize; for i in 1..steps{let t=t0+span*i as f32/steps as f32; draw_chevron(buf,w,r,t,rgb,a,scale,false);} } }
...
fn draw_arc_ccw(buf:&mut[u8],w:usize,r:f32,t0:f32,t1:f32,rgb:[u8;3],a:f32,thick:i32){ let mut end=t1; while end>=t0{end-=1.0;} let span=(t0-end).abs(); let steps=(span*180.0).ceil().max(8.0)as usize; let mut prev=point_on_circle(r,t0); for i in 1..=steps{let t=t0-span*i as f32/steps as f32; let p=point_on_circle(r,norm_t(t)); thick_line(buf,w,prev.x.round()as i32,prev.y.round()as i32,p.x.round()as i32,p.y.round()as i32,rgb,a,thick); prev=p;}}
fn draw_base_weave(buf:&mut[u8],w:usize){ for y in 0..CARPET_H{ for x in 0..w{ let n=hash01((x as u32).wrapping_mul(73856093)^(y as u32).wrapping_mul(19349663)); let warp=if x%4<2{1.08}else{0.92}; let weft=if y%4<2{1.04}else{0.96}; let base=[6.0,5.0,4.0]; put_px(buf,w,x,y,[(base[0]*warp*weft+n*3.0)as u8,(base[1]*warp*weft+n*2.5)as u8,(base[2]*warp*weft+n*2.0)as u8]); } } }
fn draw_outer_hilbert(buf:&mut[u8],w:usize,score:&WeaveScore){ let pts=fit_points(&hilbert_points(6),-(w as f32)*0.5,-(CARPET_H as f32)*0.5,(w as f32)*1.5,(CARPET_H as f32)*1.5,0.0); let clear_r2=(BEAT_R+34.0)*(BEAT_R+34.0); let x1=w as f32-CARPET_X1_PAD; for seg in pts.windows(2){ let a=seg[0]; let b=seg[1]; let m=a.add(b).mul(0.5); if m.x<CARPET_X0||m.x>x1||m.y<CARPET_Y0||m.y>CARPET_Y1{continue;} let dx=m.x-CX; let dy=m.y-CY; if dx*dx+dy*dy<=clear_r2{continue;} let t=angle_t(m.x,m.y); let phrase_idx=((t*score.phrases.len().max(1)as f32).floor()as usize).min(score.phrases.len().saturating_sub(1)); let phrase_id=score.phrases.get(phrase_idx).map(|p|p.phrase_id).unwrap_or(0); let c=dark(phrase_color(phrase_id),0.55); thick_line(buf,w,a.x.round()as i32,a.y.round()as i32,b.x.round()as i32,b.y.round()as i32,dark(c,0.75),0.24,5); thick_line(buf,w,a.x.round()as i32,a.y.round()as i32,b.x.round()as i32,b.y.round()as i32,c,0.42,3); } }
fn draw_ring_score(buf:&mut[u8],w:usize,score:&WeaveScore){ dot(buf,w,CX.round()as i32,CY.round()as i32,INNER_R.round()as i32+14,[0,0,0],0.78); dot(buf,w,CX.round()as i32,CY.round()as i32,INNER_R.round()as i32-4,[6,5,4],0.64); let layout=score_border_layout(score); for phrase in &score.phrases{ let ticks:Vec<_>=layout.iter().filter(|t|t.phrase_id==phrase.phrase_id).collect(); if let(Some(first),Some(last))=(ticks.first(),ticks.last()){let color=phrase_color(phrase.phrase_id); let lane=dark(color,0.34); draw_arc(buf,w,INNER_R,first.start_t,last.end_t,lane,0.84,13); draw_arc(buf,w,INNER_R-16.0,first.start_t,last.end_t,[22,17,12],0.68,4); draw_chevrons(buf,w,INNER_R,first.start_t,last.end_t,light(lane,0.18),0.54,6.0,false);} } for tick in layout{ let c=if tick.is_kick{[194,142,80]}else{light(dark(phrase_color(tick.phrase_id),0.20),0.08)}; dot(buf,w,tick.x.round()as i32,tick.y.round()as i32,if tick.is_kick{12}else{7},[6,4,3],0.96); dot(buf,w,tick.x.round()as i32,tick.y.round()as i32,if tick.is_kick{8}else{5},c,0.90); } }
fn draw_jump_arrows(buf:&mut[u8],w:usize,phrases:&[Phrase],score:&WeaveScore){ for(order,route)in jump_routes(phrases,score).into_iter().enumerate(){ let r=(INNER_R-22.0-order as f32*18.0).max(58.0); let mut target_t=route.target_t; while target_t>=route.source_t{target_t-=1.0;} let color=if order%2==0{[170,126,72]}else{[150,104,60]}; draw_arc_ccw(buf,w,r,route.source_t,target_t,[5,4,3],0.92,9); draw_arc_ccw(buf,w,r,route.source_t,target_t,color,0.84,5); draw_chevrons(buf,w,r,route.source_t,target_t,light(color,0.24),0.76,7.5,true); let tip=point_on_circle(BEAT_R,norm_t(target_t)); dot(buf,w,tip.x.round()as i32,tip.y.round()as i32,5,color,0.88); } }
...
fn draw_frame(buf:&mut[u8],w:usize){ for x in(24..w-24).step_by(8){let len=15+(hash01(x as u32)*18.0)as i32; line(buf,w,x as i32,28,x as i32+(hash01(x as u32+7)*12.0-6.0)as i32,28-len,[84,70,52],0.58); line(buf,w,x as i32,692,x as i32+(hash01(x as u32+13)*12.0-6.0)as i32,692+len,[84,70,52],0.58);} for y in(36..CARPET_H-36).step_by(8){let len=12+(hash01(y as u32+99)*16.0)as i32; line(buf,w,38,y as i32,38-len,y as i32+(hash01(y as u32+3)*12.0-6.0)as i32,[84,70,52],0.50); line(buf,w,w as i32-38,y as i32,w as i32-38+len,y as i32+(hash01(y as u32+5)*12.0-6.0)as i32,[84,70,52],0.50);} thick_line(buf,w,52,40,w as i32-52,40,[50,34,22],0.62,8); thick_line(buf,w,52,680,w as i32-52,680,[50,34,22],0.62,8); thick_line(buf,w,52,40,52,680,[50,34,22],0.62,8); thick_line(buf,w,w as i32-52,40,w as i32-52,680,[50,34,22],0.62,8); }

pub fn write_carpet_background(path: impl AsRef<std::path::Path>, _entries: &[CarpetEntry], phrases: &[Phrase]) -> anyhow::Result<CarpetRenderInfo>{ let score=WeaveScore::from_phrases(phrases); let mut buf=vec![0u8;W*CARPET_H*3]; draw_base_weave(&mut buf,W); draw_outer_hilbert(&mut buf,W,&score); draw_ring_score(&mut buf,W,&score); draw_jump_arrows(&mut buf,W,phrases,&score); draw_center_gosper(&mut buf,W,&score); draw_ring_score(&mut buf,W,&score); draw_phrase_labels(&mut buf,W,phrases,&score); draw_frame(&mut buf,W); let path=path.as_ref(); let mut f=std::fs::File::create(path)?; write!(f,"P6\n{} {}\n255\n",W,CARPET_H)?; f.write_all(&buf)?; f.flush()?; Ok(CarpetRenderInfo{path:path.to_string_lossy().replace('\\',"/"),width:W,content_end_x:W}) }
