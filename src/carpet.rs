...
fn draw_label_box(buf: &mut [u8], w: usize, x: i32, y: i32, title: &str, ratios: &str) {
    let ts = 2; let rs = 1; let tw = text_width(title, ts).max(text_width(ratios, rs)).max(48); let bw = tw + 16; let bh = 8 + 7*ts + 5 + 7*rs + 8; let mut x0 = x - bw/2; let mut y0 = y - bh/2;
    ...
    draw_text(buf,w,title,x0+8,y0+7,ts,[150,112,54],0.78); draw_text(buf,w,ratios,x0+8,y0+7+7*ts+5,rs,[118,88,42],0.72);
}
