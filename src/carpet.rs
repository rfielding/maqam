...
fn draw_center_gosper(buf: &mut [u8], w: usize) { let pts = fit_points_to_circle(&gosper_points(4, 1.0), CX, CY, INNER_R - 30.0, 10.0); let clip = INNER_R - 20.0; let clip2 = clip * clip; for seg in pts.windows(2) { let a=seg[0]; let b=seg[1]; let mx=(a.x+b.x)*0.5; let my=(a.y+b.y)*0.5; let dx=mx-CX; let dy=my-CY; if dx*dx+dy*dy > clip2 { continue; } thick_line(buf,w,a.x.round()as i32,a.y.round()as i32,b.x.round()as i32,b.y.round()as i32,[0,0,0],0.40,3); thick_line(buf,w,a.x.round()as i32,a.y.round()as i32,b.x.round()as i32,b.y.round()as i32,[74,54,24],0.30,1); } }
