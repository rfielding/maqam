...
fn draw_label_box(buf: &mut [u8], w: usize, x: i32, y: i32, title: &str, ratios: &str) {
    ...
    let bw = tw + 16;
    let bh = 8 + 7 * ts + 5 + 7 * rs + 8;
    let mut x0 = x - bw / 2;
    let mut y0 = y - bh / 2;
    x0 = x0.clamp(74, w as i32 - bw - 74);
    y0 = y0.clamp(58, CARPET_H as i32 - bh - 58);
    for yy in y0..y0 + bh {
        for xx in x0..x0 + bw {
            blend_px(buf, w, xx, yy, [0, 0, 0], 0.96);
        }
    }
    thick_line(buf, w, x0, y0, x0 + bw, y0, [48, 36, 18], 0.82, 2);
    thick_line(buf, w, x0, y0 + bh, x0 + bw, y0 + bh, [48, 36, 18], 0.82, 2);
    thick_line(buf, w, x0, y0, x0, y0 + bh, [48, 36, 18], 0.82, 2);
    thick_line(buf, w, x0 + bw, y0, x0 + bw, y0 + bh, [48, 36, 18], 0.82, 2);
...
fn fit_points(pts: &[Pt], x0: f32, y0: f32, x1: f32, y1: f32, margin: f32) -> Vec<Pt> {
    ...
    pts.iter()
        .map(|p| Pt::new(ox + p.x * sc, oy + p.y * sc))
        .collect()
}
fn norm_t(t: f32) -> f32 {
    t.rem_euclid(1.0)
}
fn angle_t(x: f32, y: f32) -> f32 {
    ((y - CY).atan2(x - CX) / TAU + 1.25).rem_euclid(1.0)
}
...
fn gosper_expand(symbols: &str) -> String {
    let mut out = String::with_capacity(symbols.len() * 8);
    for ch in symbols.chars() {
        match ch {
            'A' => out.push_str("A-B--B+A++AA+B-"),
            'B' => out.push_str("+A-BB--B-A++A+B"),
            '+' | '-' => out.push(ch),
            _ => {}
        }
    }
...
fn gosper_points(levels: usize, step: f32) -> Vec<Pt> {
    let mut prog = "A".to_string();
    for _ in 0..levels {
        prog = gosper_expand(&prog);
    }
    let mut pts = Vec::new();
    let mut p = Pt::new(0.0, 0.0);
    let mut ang = 0.0f32;
    let turn = std::f32::consts::PI / 3.0;
    pts.push(p);
    for ch in prog.chars() {
        match ch {
            'A' | 'B' => {
                p = Pt::new(p.x + step * ang.cos(), p.y + step * ang.sin());
                pts.push(p);
            }
            '+' => ang += turn,
            '-' => ang -= turn,
...
fn fit_points_to_circle(pts: &[Pt], cx: f32, cy: f32, r: f32, margin: f32) -> Vec<Pt> {
    let minx = pts.iter().map(|p| p.x).fold(f32::INFINITY, f32::min);
    let maxx = pts.iter().map(|p| p.x).fold(f32::NEG_INFINITY, f32::max);
    let miny = pts.iter().map(|p| p.y).fold(f32::INFINITY, f32::min);
    let maxy = pts.iter().map(|p| p.y).fold(f32::NEG_INFINITY, f32::max);
    let scale = ((2.0 * (r - margin)) / (maxx - minx).max(maxy - miny).max(0.001)).max(0.001);
    let ox = cx - ((minx + maxx) * 0.5) * scale;
    let oy = cy - ((miny + maxy) * 0.5) * scale;
    pts.iter()
...
fn draw_chevron(
    ...
    let ax = p.x + dx * scale;
    let ay = p.y + dy * scale;
    let bx = p.x - dx * scale * 0.8;
    let by = p.y - dy * scale * 0.8;
    let l = Pt::new(bx + nx * scale * 0.65, by + ny * scale * 0.65);
    let rr = Pt::new(bx - nx * scale * 0.65, by - ny * scale * 0.65);
    thick_line(
        buf,
        w,
        l.x.round() as i32,
...
fn draw_chevrons(
    ...
    if ccw {
        ...
        let span = (t0 - end).abs();
        let steps = (span * 70.0).ceil().max(3.0) as usize;
        for i in 1..steps {
            let t = t0 - span * i as f32 / steps as f32;
            draw_chevron(buf, w, r, t, rgb, a, scale, true);
        }
    } else {
        let mut end = t1;
        if end < t0 {
        ...
        for i in 1..steps {
            let t = t0 + span * i as f32 / steps as f32;
            draw_chevron(buf, w, r, t, rgb, a, scale, false);
        }
    }
}
...
fn draw_arc_ccw(
    ...
    let span = (t0 - end).abs();
    let steps = (span * 180.0).ceil().max(8.0) as usize;
    let mut prev = point_on_circle(r, t0);
    for i in 1..=steps {
        let t = t0 - span * i as f32 / steps as f32;
        let p = point_on_circle(r, norm_t(t));
    ...
    for y in 0..CARPET_H {
        for x in 0..w {
            ...
            put_px(
                ...
                y,
                [
                    (base[0] * warp * weft + n * 3.0) as u8,
                    (base[1] * warp * weft + n * 2.5) as u8,
                    (base[2] * warp * weft + n * 2.0) as u8,
                ],
            );
        }
    }
...
fn draw_outer_hilbert(buf: &mut [u8], w: usize, score: &WeaveScore) {
    ...
    let x1 = w as f32 - CARPET_X1_PAD;
    for seg in pts.windows(2) {
        let a = seg[0];
        let b = seg[1];
        let m = a.add(b).mul(0.5);
        if m.x < CARPET_X0 || m.x > x1 || m.y < CARPET_Y0 || m.y > CARPET_Y1 {
            continue;
        }
        let dx = m.x - CX;
        let dy = m.y - CY;
        ...
        let phrase_idx = ((t * score.phrases.len().max(1) as f32).floor() as usize)
            .min(score.phrases.len().saturating_sub(1));
        let phrase_id = score
            .phrases
            .get(phrase_idx)
            .map(|p| p.phrase_id)
            .unwrap_or(0);
        let c = dark(phrase_color(phrase_id), 0.55);
        thick_line(
            buf,
            w,
            a.x.round() as i32,
...
fn draw_ring_score(buf: &mut [u8], w: usize, score: &WeaveScore) {
    ...
    for phrase in &score.phrases {
        let ticks: Vec<_> = layout
            .iter()
            .filter(|t| t.phrase_id == phrase.phrase_id)
            .collect();
        if let (Some(first), Some(last)) = (ticks.first(), ticks.last()) {
            let color = phrase_color(phrase.phrase_id);
            let lane = dark(color, 0.34);
            draw_arc(buf, w, INNER_R, first.start_t, last.end_t, lane, 0.84, 13);
            draw_arc(
                buf,
                w,
                INNER_R - 16.0,
    ...
    for tick in layout {
        let c = if tick.is_kick {
            ...
            light(dark(phrase_color(tick.phrase_id), 0.20), 0.08)
        };
        dot(
            buf,
            w,
...
fn draw_jump_arrows(buf: &mut [u8], w: usize, phrases: &[Phrase], score: &WeaveScore) {
    for (order, route) in jump_routes(phrases, score).into_iter().enumerate() {
        ...
        let tip = point_on_circle(BEAT_R, norm_t(target_t));
        dot(
            buf,
            w,
            tip.x.round() as i32,
...
fn phrase_label(phrase: &Phrase) -> (String, String) {
    let title = phrase
        .src
        .split(',')
        .filter_map(|part| {
            let tokens: Vec<&str> = part
                .split_whitespace()
                .filter(|s| !s.chars().all(|c| c.is_ascii_digit()) && !s.starts_with('r'))
                .collect();
    ...
    let title = if title.is_empty() {
        if !phrase.bar.maqam_names.is_empty() {
            phrase.bar.maqam_names.join("+").to_uppercase()
        } else {
            phrase
                .src
                .split_whitespace()
...
fn draw_phrase_labels(buf: &mut [u8], w: usize, phrases: &[Phrase], score: &WeaveScore) {
    let layout = score_border_layout(score);
    for phrase in &score.phrases {
        let Some(src) = phrases.iter().find(|p| p.id == phrase.phrase_id) else {
            continue;
        };
        let ticks: Vec<_> = layout
            .iter()
            ...
            .collect();
        if let (Some(first), Some(last)) = (ticks.first(), ticks.last()) {
            let mid = norm_t((first.start_t + last.end_t) * 0.5);
            let p = point_on_circle(LABEL_R, mid);
            let (title, ratios) = phrase_label(src);
            draw_label_box(
                buf,
                w,
                p.x.round() as i32,
...
fn draw_frame(buf: &mut [u8], w: usize) {
    for x in (24..w - 24).step_by(8) {
        ...
        line(
            ...
            x as i32,
            28,
            x as i32 + (hash01(x as u32 + 7) * 12.0 - 6.0) as i32,
            28 - len,
            [84, 70, 52],
            0.58,
        );
        line(
            ...
            x as i32,
            692,
            x as i32 + (hash01(x as u32 + 13) * 12.0 - 6.0) as i32,
            692 + len,
            [84, 70, 52],
            0.58,
        );
    ...
    for y in (36..CARPET_H - 36).step_by(8) {
        ...
        line(
            ...
            y as i32,
            38 - len,
            y as i32 + (hash01(y as u32 + 3) * 12.0 - 6.0) as i32,
            [84, 70, 52],
            0.50,
        );
        line(
            ...
            y as i32,
            w as i32 - 38 + len,
            y as i32 + (hash01(y as u32 + 5) * 12.0 - 6.0) as i32,
            [84, 70, 52],
            0.50,
        );
    }
    thick_line(buf, w, 52, 40, w as i32 - 52, 40, [50, 34, 22], 0.62, 8);
    thick_line(buf, w, 52, 680, w as i32 - 52, 680, [50, 34, 22], 0.62, 8);
    thick_line(buf, w, 52, 40, 52, 680, [50, 34, 22], 0.62, 8);
    thick_line(
        buf,
        w,
        w as i32 - 52,
