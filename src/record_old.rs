...
fn build_carpet_tick_highlights(
    ...
    for entry in full_seq {
        ...
        for subdivision in 0..phrase.bar.events.len() {
            ...
            parts.push(format!(
                "drawbox=x={xo}:y={yo}:w={outer}:h={outer}:color=0x44FF88@0.20:t=fill:enable='between(t,{start:.6},{end:.6})',\
                 drawbox=x={xi}:y={yi}:w={inner}:h={inner}:color=0xD8FFAA@0.82:t=fill:enable='between(t,{start:.6},{end:.6})'"
            ));
            if subdivision == 0 {
...
pub fn record_cycle(
    ...
    for (seq_pos, entry) in full_seq.iter().enumerate() {
        ...
        if is_first {
            ...
            let phrase_secs = (phrases_v[phrase_idx].bar.total_subdivs as f64 * subdiv_secs * repeats as f64).min(3.0);
        ...
        crate::REC_SAMPLES_DONE.store(left_buf.len().min(render_samples), std::sync::atomic::Ordering::Relaxed);
    ...
    let peak = left_buf.iter().chain(right_buf.iter()).map(|s| s.abs()).fold(0f32, f32::max);
        ...
        for (i, entry) in full_seq.iter().enumerate() {
            ...
            for (pi, p) in phrases.iter().enumerate() {
                ...
                if let Some(js) = &p.jump {
                    ...
                    let (pass, total) = snap.and_then(|s| s.get(&p.id)).copied().unwrap_or((0, js.times));
                    ...
                    for si in 0..n {
                        ...
                        let body = format!("{:<20} {}{} {:<16} {:<7}", p.src, rhy, pad, maqam_str, repeat_display);
